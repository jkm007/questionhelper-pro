...
export const toggleQuestionBookmark = (practiceId: number, questionId: number) => {
  return request({ url: `/practices/${practiceId}/bookmark`, method: 'POST', data: { questionId } })
}

// 暂停练习
export const pausePractice = (id: number) => {
  return request({ url: `/practices/${id}/pause`, method: 'POST' })
}

// 恢复练习
export const resumePractice = (id: number) => {
  return request({ url: `/practices/${id}/resume`, method: 'POST' })
}

// 结束练习
export const finishPractice = (id: number) => {
  return request({ url: `/practices/${id}/finish`, method: 'POST' })
}
