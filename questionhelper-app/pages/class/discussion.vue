<template>
  <view class="page">
    <scroll-view
      class="list-container"
      scroll-y
      refresher-enabled
      :refresher-triggered="refreshing"
      @refresherrefresh="onRefresh"
      @scrolltolower="onLoadMore"
    >
      <view
        v-for="item in list"
        :key="item.id"
        class="discuss-card"
        @tap="goDetail(item.id)"
      >
        <view v-if="item.isPinned" class="pin-badge">置顶</view>
        <text class="discuss-title">{{ item.title }}</text>
        <view class="discuss-meta">
          <text class="meta-author">{{ item.authorName }}</text>
          <text class="meta-time">{{ item.createdAt }}</text>
          <text class="meta-replies">{{ item.replyCount }} 回复</text>
        </view>
      </view>

      <view v-if="!loading && list.length === 0" class="empty-state">
        <text class="empty-text">暂无讨论</text>
      </view>

      <view v-if="list.length > 0" class="load-more">
        <text v-if="loading" class="load-more-text">加载中...</text>
        <text v-else-if="noMore" class="load-more-text">没有更多了</text>
      </view>
    </scroll-view>

    <view v-if="isTeacher" class="fab" @tap="goCreate">
      <text class="fab-icon">+</text>
    </view>
  </view>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue'

interface DiscussionItem {
  id: string
  title: string
  authorName: string
  createdAt: string
  isPinned: boolean
  replyCount: number
}

const classId = ref('')
const isTeacher = ref(false)
const list = ref<DiscussionItem[]>([])
const loading = ref(false)
const refreshing = ref(false)
const noMore = ref(false)
const page = ref(1)
const pageSize = 20

async function loadData(reset = false) {
  if (loading.value) return
  if (reset) {
    page.value = 1
    noMore.value = false
    list.value = []
  }
  loading.value = true
  try {
    // TODO: import { getClassDiscussions } from '@/api/class'
    // const res = await getClassDiscussions({ classId: classId.value, page: page.value, pageSize })
    const newData: DiscussionItem[] = []
    if (reset) list.value = newData
    else list.value = [...list.value, ...newData]
    noMore.value = newData.length < pageSize
    page.value++
  } catch (e) {
    uni.showToast({ title: '加载失败', icon: 'none' })
  } finally {
    loading.value = false
  }
}

async function onRefresh() {
  refreshing.value = true
  await loadData(true)
  refreshing.value = false
}

function onLoadMore() {
  if (!noMore.value && !loading.value) loadData()
}

function goDetail(id: string) {
  uni.navigateTo({ url: `/pages/class/discussion-detail?id=${id}` })
}

function goCreate() {
  uni.navigateTo({ url: `/pages/class/discussion-create?classId=${classId.value}` })
}

onMounted(() => {
  const pages = getCurrentPages()
  const current = pages[pages.length - 1] as any
  classId.value = current.options?.classId || ''
  isTeacher.value = uni.getStorageSync('role') === 'teacher'
  loadData(true)
})
</script>

<style scoped>
.page {
  min-height: 100vh;
  background-color: #f5f6fa;
}

.list-container {
  height: 100vh;
  padding: 20rpx 24rpx;
}

.discuss-card {
  background-color: #fff;
  border-radius: 16rpx;
  padding: 24rpx;
  margin-bottom: 20rpx;
  box-shadow: 0 2rpx 12rpx rgba(0, 0, 0, 0.04);
  position: relative;
}

.pin-badge {
  display: inline-block;
  font-size: 20rpx;
  color: #ff6a00;
  background-color: #fff3e6;
  padding: 4rpx 12rpx;
  border-radius: 8rpx;
  margin-bottom: 12rpx;
}

.discuss-title {
  font-size: 30rpx;
  font-weight: 600;
  color: #333;
  margin-bottom: 12rpx;
  display: block;
}

.discuss-meta {
  display: flex;
  align-items: center;
  gap: 24rpx;
}

.meta-author {
  font-size: 24rpx;
  color: #666;
}

.meta-time {
  font-size: 24rpx;
  color: #999;
}

.meta-replies {
  font-size: 24rpx;
  color: #1677ff;
}

.empty-state {
  display: flex;
  align-items: center;
  justify-content: center;
  padding: 120rpx 0;
}

.empty-text {
  font-size: 28rpx;
  color: #999;
}

.load-more {
  padding: 24rpx 0;
  text-align: center;
}

.load-more-text {
  font-size: 24rpx;
  color: #999;
}

.fab {
  position: fixed;
  right: 40rpx;
  bottom: 80rpx;
  width: 100rpx;
  height: 100rpx;
  border-radius: 50%;
  background-color: #1677ff;
  display: flex;
  align-items: center;
  justify-content: center;
  box-shadow: 0 8rpx 24rpx rgba(22, 119, 255, 0.4);
}

.fab-icon {
  font-size: 48rpx;
  color: #fff;
  font-weight: 300;
}
</style>
