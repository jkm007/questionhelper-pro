...
| POST | `/api/v1/admin/questions/batch-import` | 批量导入题目（按分类/标签筛选条件导入） |
| POST | `/api/v1/admin/questions/batch-export` | 批量导出题目（按指定ID列表导出） |
...
| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/api/v1/admin/question-bank/options` | 获取题库下拉选项列表（供表单选择器使用） |
